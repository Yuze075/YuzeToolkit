...
{
    /// <summary>
    /// MVC中的<see cref="IModel"/>层，用于存储数据，将逻辑和数据分离<br/>
    /// <see cref="ICommand{TResult}"/>会<see cref="IModel"/>进行操作；
    /// <see cref="IModel"/>通过Event想<see cref="IController"/>传递信息
    /// </summary>
    public interface IModel : ICanSendEvent, ICanRegisterEvent, ICanGetModel, ICanGetUtility
    {
    }

    /// <summary>
    /// 控制器的接口，产生<see cref="ICommand{TResult}"/>对<see cref="IModel"/>中数据操作;
    /// 同时接受<see cref="IModel"/>发送的Event, 进行显示的更新<br/>
    /// 通常View（UGUI）和<see cref="IController"/>会处于统一层级，不做过多的区分。
    /// </summary>
    public interface IController : ICanSendCommand, ICanRegisterEvent, ICanGetModel
    {
    }
    ...
    /// 通常<see cref="IModel"/>,<see cref="IController"/>,<see cref="ICommand{TResult}"/>都继承自这个接口<br/>
    /// 但<see cref="IUtility"/>不继承自这个接口, 作为外部访问接口不直接属于<see cref="IUICore"/><br/>
    /// (虽然大部分情况下都属于<see cref="IUICore"/>, 但<see cref="IUtility"/>不需要使用拓展方法就没有继承)
    /// </summary>
    ...
    public static class Rule
    {
        ...
        public static TModel GetNotNullModel<TModel>(this ICanGetModel self, string? name = null,
            string? message = null, bool additionalCheck = true) where TModel : IModel =>
        ...
        public static TUtility GetNotNullUtility<TUtility>(this ICanGetUtility self, string? name = null,
            string? message = null, bool additionalCheck = true) where TUtility : IUtility  =>
            self.Core.GetNotNullUtility<TUtility>(name, message, additionalCheck);

        public static Action<T> ToAction<T, TR>(this Func<T, TR> func) => t => { func.Invoke(t); };
    }
}