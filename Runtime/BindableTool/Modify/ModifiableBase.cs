...
{
    [Serializable]
    public abstract class ModifiableBase<TValue, TModify> : BindableBase<TValue>, IModifiable where TModify : IModify
    {
        protected ModifiableBase(bool isReadOnly, IModifiableOwner? modifiableOwner, ILogging? loggingParent) :
            base(loggingParent ?? modifiableOwner as ILogging)
        {
            _owner = modifiableOwner;
            this.isReadOnly = isReadOnly;
        }

        ~ModifiableBase() => Dispose(false);
