...
namespace YuzeToolkit.Editor
{
    public static class ReflectionHelperEditor
    {
        public static void DrawButton(this SerializedProperty? property, Rect rect, string methodName,
            string? buttonName = null, int upLayers = 1)
        {
            if (property == null) return;
            var target = property.GetPropertyOwnerObject(upLayers);
            if (target == null) return;
            var methodInfos = target.GetType().GetAllMethods(methodName);

            // 如果没有对应函数, 提示错误
            var enumerable = methodInfos as MethodInfo[] ?? methodInfos.ToArray();
            if (enumerable.Length <= 0)
            {
                DrawWarningMessage(rect, property.displayName + "(无法找到对应函数!)");
                return;
            }
            ...
            if (methodInfo == null)
            {
                DrawWarningMessage(rect, property.displayName + "(不存在无参函数!)");
                EditorGUI.EndProperty();
                return;
            }

            if (GUI.Button(rect, buttonName ?? methodInfo.Name)) methodInfo.Invoke(target, null);
        }
        ...
        public static object? GetValue(this SerializedProperty? property, string? valueName) =>
            ReflectionHelper.GetValue(property.GetPropertyOwnerObject(), valueName);
        ...
        public static bool SetValue(this SerializedProperty? property, string? valueName, object? value) =>
            ReflectionHelper.SetValue(property.GetPropertyOwnerObject(), valueName, value);
